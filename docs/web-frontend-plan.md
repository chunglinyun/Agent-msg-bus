...
但「痛了再換 marked」這條退路**在串流上線後就變窄**：串流要求渲染器容忍未閉合結構，marked 的收斂方式未必是我們要的，屆時換過去不是刪 40 行，而是外面再包一層前處理。**順序上先確定串流做不做，再決定渲染器。**
...
**解法：每收到一個 chunk 就把累積到目前的整個 buffer 重新完整 parse，不做增量／狀態式 parser。** buffer 只有幾 KB，重 parse 成本可忽略；增量 parser 要維護跨 chunk 狀態，是所有 bug 的來源。

1. 渲染器必須容忍未閉合結構——未閉合的 `` ``` `` 視為開著的 code block，其後全當程式碼；下一個 chunk 進來重 parse 自然收斂。
2. 未閉合的行內標記（`` ` ``、`**`）保持原字元，不隱式閉合，避免畫面跳動。
3. 重 parse 用 `requestAnimationFrame` 節流，且只重繪最後一則 bubble。
